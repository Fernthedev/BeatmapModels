...
{
    public V2Slider(IDictionary<string, JToken>? unserializedData, float time, V2SliderCustomData? typedCustomData,
        int lineIndex, int type, int cutDirection, int lineLayer, int colorType, float headTime, int headLineIndex,
        int headLineLayer, float headControlPointLengthMultiplier, int headCutDirection, float tailTime,
        int tailLineIndex, int tailLineLayer, float tailControlPointLengthMultiplier, int tailCutDirection,
        int sliderMidAnchorMode) : base(unserializedData, time, typedCustomData, lineIndex)
    {
        Type = type;
    ...
    public override IBeatmapJSON Clone()
    {
        return new V2Slider(new Dictionary<string, JToken>(UnserializedData), Time,
            TypedCustomData?.Clone() as V2SliderCustomData, LineIndex, Type, CutDirection, LineLayer, ColorType,
            HeadTime,
            HeadLineIndex,
