...
{
    [JsonConstructor]
    public V2Beatmap(IDictionary<string, JToken>? unserializedData, string? version, IList<INote> notes,
        IList<IBasicEvent> events, IList<IObstacle> obstacles, IList<IWaypoint> waypoints, IList<ISlider>? sliders,
    ...
    [JsonIgnore] public bool UseNormalEventsAsCompatibleEvents { get; set; } = true;
    ...
    [JsonIgnore]
    public IList<IBomb> Bombs
    {
        get => Notes.Where(e => e.Type == (int)V2NoteType.Bomb).Cast<IBomb>().ToList();
        set => Notes = Notes.Where(e => e.Type != (int)V2NoteType.Bomb).Union(value.Cast<INote>()).ToList();
    }
